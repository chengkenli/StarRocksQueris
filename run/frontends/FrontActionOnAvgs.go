...
func (w *Workers) handleOnAvgs(db *gorm.DB, app, fe string, queries util.Queris, item *util.Process2) error {
	if util.ConnectNorm["slow_query_frontend_avgs"] == nil {
		return nil
	}
	...
	_, ok := FsCache.Get(fmt.Sprintf("%d_%s", 1, item.Id))
	if ok {
		return nil
	}
	...
	for _, match := range matches {
		...
		frontendAvgs := util.ConnectNorm["slow_query_frontend_avgs"].(string)
