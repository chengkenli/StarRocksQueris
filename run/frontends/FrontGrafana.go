...
func FrontGrafana(g *util.Grafana) {
	if util.ConnectNorm["slow_query_grafana"] == nil {
		return
	}
	if util.P.Check {
		return
	...
	body := fmt.Sprintf(`
{
	...
	//发送POST请求并处理响应
	response, err := Client.R().SetHeader("Content-Type", "application/json").SetBody(body).Post(util.ConnectNorm["slow_query_grafana"].(string))
	if err != nil {
		util.Loggrs.Error(err.Error())
		return
	}
	util.Loggrs.Info(body)
