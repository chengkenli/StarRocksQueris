...
func InProcess(i *util.InQue) (*util.Larkbodys, *util.SchemaData) {
	nt := time.Now()
	defer func() {
		util.Loggrs.Info(fmt.Sprintf("分析进程信息 %s %v", i.Item.Id, time.Now().Sub(nt).String()))
	}()
	// end
	go QuerusFile(i)
	// 发送飞书提醒
	...
	if i.Schema != nil {
		...
		if len(info.EmailTo) != 0 {
			if util.ConnectNorm["slow_query_email_suffix"] != nil {
				owner = strings.ReplaceAll(info.EmailTo[0], util.ConnectNorm["slow_query_email_suffix"].(string), "")
			}
		}
	...
	// 发送邮件提醒
	go func() {
		_, Ok := i.EmCache.Get(fmt.Sprintf("email_%s", i.Item.Id))
		if Ok {
			return
		}
		robot.SendEmQueris(
