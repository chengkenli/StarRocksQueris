...
func SendFsQueris(i *util.InQue, queris bool) (*util.Larkbodys, error) {
	...
	if i.Item.Host != "" {
		...
		if util.ClientIPDec != nil {
			ip := strings.Split(i.Item.Host, ":")[0]
			// 根据ip获取域名进行匹配
			n, err := net.LookupAddr(ip)
			if err != nil {
				util.Loggrs.Warn(uid, err.Error())
			}
			util.Loggrs.Info(uid, fmt.Sprintf("LookupAddr :%v, length:%d", n, len(n)))
			if n != nil {
				h1 := strings.ToLower(strings.Split(n[0], ".")[0])
				for _, data := range util.ClientIPDec {
					h2 := strings.ToLower(strings.Split(data.SystemName, ".")[0])
					if h1 == h2 && len(data.User) != 0 {
						util.Loggrs.Info(uid, fmt.Sprintf("LookupAddr client:<%s>,database:<%s>", h1, h2))
						address := strings.Split(data.SystemName, ".")[0]
						submitu := strings.ReplaceAll(data.User, " ", "")
						msgs = append(msgs, fmt.Sprintf(`💬Submit User:\t\t[**%s**]\n`, submitu))
						msgs = append(msgs, fmt.Sprintf(`💬Submit Address:\t[**%s/%s**]\n`, address, data.Manufacturer))
						break
					}
				}
			}
