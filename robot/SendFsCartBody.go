...
// SendFsCartBody session
func SendFsCartBody(body []*util.Larkbodys) {
	if len(util.ConnectRobot) == 0 {
		return
	}

	go SendFsCartGlobal(body)
	if !util.P.Check {
		go SendFsCartID(body)
		go SendFsCartApp(body)
	}
	for _, m := range util.ConnectRobot {
		if m["type"] != nil {
			...
			util.Loggrs.Info(fmt.Sprintf("通过[集群名称]分发给不同的群组"))
