...
func SessionSchemaRegexpOwner(uid int, schema []string) *util.EmailMain {
			...
			for _, d := range util.Domain {
				...
				for _, u := range strings.Split(d[zty], ",") {
					if util.ConnectNorm["slow_query_email_suffix"] != nil {
						email = append(email, fmt.Sprintf("%s%s", u, util.ConnectNorm["slow_query_email_suffix"].(string)))
					} else {
	...
	if uid == 1 {
		if email != nil {
			if util.ConnectNorm["slow_query_email_suffix"] != nil {
				cc = SchemaDomainGroup(strings.ReplaceAll(email[0], util.ConnectNorm["slow_query_email_suffix"].(string), ""))
			}
		}
	}
...
func SchemaDomainGroup(leader string) []string {
	...
	for _, s := range strings.Split(g.ID, ",") {
		if util.ConnectNorm["slow_query_email_suffix"] != nil {
			email = append(email, s+util.ConnectNorm["slow_query_email_suffix"].(string))
		}
	}
	return email
}
